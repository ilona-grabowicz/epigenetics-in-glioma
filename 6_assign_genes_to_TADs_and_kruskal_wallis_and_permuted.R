...
############# Assigning genes present (in all datasets) to TADs and calculating kruskal-wallis test:
# For single analysis:
# dataset <- 'DNAme'
...
funkcja <- function(dataset, comparison, part) {
  ...
  # Subsetting to the same genes in all datasets - not all datasets contained information about all the genes:
  ...
  DE <- DE[DE$ENSEMBL%in%DNAme_genes$gene,] # Limiting to genes included in DNA methylation data
  DE <- DE[DE$ENSEMBL%in%DNAme_proms$gene,]
...
############# Kruskal-Wallis test on permuted data:
...
funkcja <- function(dataset, comparison, part) {
  ...
  for (perm in 1:permutations){
    DE$random <- sample(DE$log2_FC, replace=FALSE)
    # Which genes are in which TADs?
    list_of_TADs <-list()
    for (row in 1:nrow(TADs)) { #nrow(TADs)
      # cat('.')
      my_DE <- DE[DE$chromosome == TADs[row,"chr"],]
      is.included <- mapply(function(de_start,de_stop,tad_start,tad_stop){de_start >= tad_start &  de_stop <= tad_stop}, my_DE$start, my_DE$stop, TADs[row,]$start, TADs[row,]$stop)
      list_of_TADs[[row]] <- my_DE[is.included,'random']
    }
    # Remove empty TADs and Kruskal-Wallis test
    list_of_TADs_noNA <- lapply(list_of_TADs, function(x) x[!is.na(x)])
    list_of_TADs_no_empty <- list_of_TADs_noNA[lapply(list_of_TADs_noNA,length)>1] # Biore pod uwage (do policzenia testu) wszystkie TADy co maja conajmniej 1 gen.
    a<- kruskal.test(list_of_TADs_no_empty)
    statistics <- c(statistics, a$statistic)
    p_vals <- c(p_vals, a$p.value)
  }
  #print(statistics)
...
# Calculate permutations:
 ...
 rnaseq <- funkcja('RNASeq', 'grade1_grade23', 'promoters')
# rnaseq <- funkcja('RNASeq', 'grade23_grade4', 'promoters')
# rnaseq <- funkcja('RNASeq', 'grade1_grade4', 'promoters')
# h3k4me3 <- funkcja('H3K4me3', 'grade1_grade4', 'promoters')
 h3k4me3 <- funkcja('H3K4me3', 'grade1_grade23', 'promoters')
# h3k4me3 <- funkcja('H3K4me3', 'grade23_grade4', 'promoters')
